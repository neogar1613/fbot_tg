...
class Categories:
    def __init__(self):
        ...
        self._base_categories = self._load_base_categories()
    ...
    def _load_base_categories(self) -> List[Category]:
        """Возвращает обязательные категории расходов из БД"""
        categories = db.fetch_where_is_based(
            "category", "codename name is_base_expense aliases".split()
        )
        categories = self._fill_aliases(categories)
        return categories
    ...
    def get_base_categories(self) -> List[Dict]:
        """Возвращает справочник обязательных категорий."""
        return self._base_categories
